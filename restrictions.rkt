...
addition every test is required to yield a boolean.                                                 |#
   ...
   (define-syntax-rule (Lambda (var0 var ...) expr) (lambda (var0 var ...) expr))
   ...
   (provide quote (rename-out (Lambda lambda) (Cond cond)))                                         #|
...
That's it, almost. We must export some #% stuff. In addition we provide procedure |show|.           |#
   ...
   (define (show x)
    (define y (if (and (list? x) (andmap null? x)) (length x) x))
    (define str (format "~s" y))
